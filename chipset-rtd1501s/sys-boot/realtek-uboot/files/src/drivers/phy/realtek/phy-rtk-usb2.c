...
static char rtk_phy_read(struct phy_reg *phy_reg, char addr)
{
	...
	reg_gusb2phyacc0 += (phy_reg->phyacc0_index * 4);
...
static int rtk_phy_write(struct phy_reg *phy_reg, char addr, char data)
{
	...
	debug("%s: wrapper 0x%x set parameter addr=0x%x value=0x%x ==> 0x%x\n",
		__func__, (int)reg_wrap_vstatus, addr, data, rtk_phy_read(phy_reg, addr));
				       ...
				       struct phy_parameter *phy_parameter, bool update)
{
	if (phy_cfg->usb_dc_disconnect_at_page0)
		update_dc_disconnect_level_at_page0(rtk_phy, phy_cfg, phy_parameter, update);
	else
		update_dc_disconnect_level_at_page1(rtk_phy, phy_cfg, phy_parameter, update);
}
				    ...
				    struct phy_parameter *phy_parameter, int index)
{
	...
	phy_cfg = &((struct phy_cfg *)rtk_phy->phy_cfg)[index];
	phy_reg = &phy_parameter->phy_reg;
...
do_toggle:
	...
	update_dc_disconnect_level(rtk_phy, phy_cfg, phy_parameter, false);
	...
	update_dc_disconnect_level(rtk_phy, phy_cfg, phy_parameter, true);
...
static int rtk_phy_init(struct phy *phy)
{
	...
	dev_dbg(rtk_phy->dev, "Initialized RTK USB 2.0 PHY\n");
				 ...
				 struct phy_parameter *phy_parameter, int index)
{
	...
	if (index)
		snprintf(name, sizeof(name), "usb-dc-cal%d", index);
	else
		snprintf(name, sizeof(name), "%s", "usb-dc-cal");

	cell = nvmem_cell_get(rtk_phy->dev, name);
	if (IS_ERR(cell)) {
		dev_dbg(rtk_phy->dev, "%s no %s: %ld\n",
			__func__, name, PTR_ERR(cell));
	} else {
		unsigned char *buf;
		size_t buf_size;
	...
	/* Read efuse for usb dc cal */
	if (index)
		snprintf(name, sizeof(name), "realtek,usb-dc-cal%d", index);
	else
		snprintf(name, sizeof(name), "%s", "realtek,usb-dc-cal");

	if (!dev_read_u32_index(dev, name, 0, &reg)) {
		int val;

		dev_read_u32_index(dev, name, 1, &offset);
		dev_read_u32_index(dev, name, 2, &mask);
	...
	if (phy_cfg->check_efuse_version == CHECK_EFUSE_V1) {
		...
		if (device_is_compatible(dev, "realtek,rtd1312c-usb2phy")) {
	...
	if (index)
		snprintf(name, 32, "usb-dc-dis%d", index);
	else
		snprintf(name, 32, "%s", "usb-dc-dis");
	...
	/* Read efuse for usb dc cal */
	if (index)
		snprintf(name, sizeof(name), "realtek,usb-dc-dis%d", index);
	else
		snprintf(name, sizeof(name), "%s", "realtek,usb-dc-dis");
...
static int parse_phy_data(struct rtk_phy *rtk_phy, struct phy_cfg *phy_cfg)
{
	struct udevice *dev = rtk_phy->dev;
	//struct device_node *np = dev_np(dev);
	struct phy_parameter *phy_parameter;
	...
	struct resource res;
	int na, ns, num_regs;
	int num_wrap, num_phyacc;
	int idx_wrap, idx_phyacc;
	...
	if (num_regs < 2) {
		dev_err(dev, "phy num_regs is not right!\n");
		return -EINVAL;
	}

	idx_wrap = 0;
	idx_phyacc = num_regs - 1;
	ret = dev_read_resource(dev, idx_phyacc, &res);
	if (ret)
		return ret;

	/* check multiple phy case */
	num_wrap = num_regs - 1;
	if (num_wrap > 1) {
		num_phyacc = (res.end - res.start + 1) / 4;
		if (num_phyacc != num_wrap) {
			dev_err(dev, "phy num_wrap is not match with num_phyacc\n");
			return -EINVAL;
		}
	} else {
		num_phyacc = (res.end - res.start + 1) / 4;
	...
	rtk_phy->phy_cfg = kzalloc(sizeof(struct phy_cfg) * rtk_phy->num_phy, GFP_KERNEL);

	for (index = 0; index < rtk_phy->num_phy; index++) {
		struct phy_cfg *__phy_cfg;

		__phy_cfg = &((struct phy_cfg *)rtk_phy->phy_cfg)[index];
		memcpy(__phy_cfg, phy_cfg, sizeof(*phy_cfg));
	}
	...
	for (index = 0; index < rtk_phy->num_phy; index++) {
		...
		if (num_wrap > 1) {
			idx_wrap = index;
			phy_parameter->phy_reg.vstatus_index = 0;
		} else {
			idx_wrap = 0;
			phy_parameter->phy_reg.vstatus_index = index;
		}
		phy_parameter->phy_reg.phyacc0_index = index;

		phy_parameter->phy_reg.reg_wrap_vstatus = dev_remap_addr_index(dev, idx_wrap);
		phy_parameter->phy_reg.reg_gusb2phyacc0 = dev_remap_addr_index(dev, idx_phyacc);

		if (dev_read_prop(dev, "realtek,inverse-hstx-sync-clock", NULL))
			phy_parameter->inverse_hstx_sync_clock = true;
		else
			phy_parameter->inverse_hstx_sync_clock = false;

		if (dev_read_u32_index(dev, "realtek,driving-level",
					       index, &phy_parameter->driving_level))
			phy_parameter->driving_level = DEFAULT_DC_DRIVING_VALUE;

		if (dev_read_u32_index(dev, "realtek,driving-level-compensate",
					       index, &phy_parameter->driving_level_compensate))
			phy_parameter->driving_level_compensate = 0;

		if (dev_read_u32_index(dev, "realtek,disconnection-compensate",
					       index, &phy_parameter->disconnection_compensate))
			phy_parameter->disconnection_compensate = 0;

		get_phy_data_by_efuse(rtk_phy, phy_parameter, index);

		update_dc_driving_level(rtk_phy, phy_parameter, index);

		update_hs_clk_select(rtk_phy, phy_parameter, index);
	}

	return ret;
}

static int rtk_usb2phy_probe(struct udevice *dev)
{
	...
	ret = parse_phy_data(rtk_phy, phy_cfg);

	return ret;
}

static int rtk_usb2phy_remove(struct udevice *dev)
{
	...
	for (index = 0; index < rtk_phy->num_phy; index++) {
		...
		unmap_physmem(phy_parameter->phy_reg.reg_wrap_vstatus, MAP_NOCACHE);
		unmap_physmem(phy_parameter->phy_reg.reg_gusb2phyacc0, MAP_NOCACHE);
	}
...
static const struct phy_cfg rtd1319_phy_cfg = {
	...
	.usb_dc_disconnect_at_page0 = true,
	.do_toggle = true,
	.do_toggle_driving = false, /* bootcode disable this */
	.driving_updated_for_dev_dis = 0xf,
	.use_default_parameter = false,
