...
#define EFUSE_OTP_REG       0x980174fc
...
#define ISO_TESTMUX_MUXPAD5 0x9804e014
#define ISO_TESTMUX_MUXPAD6 0x9804e018
#define ISO_TESTMUX_PFUNC9  0x9804e040  /* MDC/MDIO current */
#define ISO_TESTMUX_PFUNC20 0x9804e06c  /* RGMII current */
#define ISO_TESTMUX_PFUNC21 0x9804e070  /* RGMII current */
#define ISO_TESTMUX_PFUNC25 0x9804e090  /* RGMII BIAS */

#elif defined(CONFIG_TARGET_RTD1619B)
#define EFUSE_OTP_REG		0x98032504
#define EFUSE_OTP_REG2		0x98032508
#define R_K_DEFAULT		0x8
#define AMP_K_DEFAULT		0x7777
#define RC_K_DEFAULT		0x8888
#define OTP_BOND_ID		0x980323C8

#define ISO_TESTMUX_MUXPAD2 0x9804e008
#define ISO_TESTMUX_PFUNC12 0x9804e050  /* MDC/MDIO current */
#elif defined(CONFIG_TARGET_RTD1625)
#define EFUSE_OTP_REG           0x98032500
#define RS_DEFAULT              0x3     /* TX */
#define RSW_DEFAULT             0x4     /* RX */
#define IDAC_FINE_MDI_DEFAULT   0x12    /* TX */
...
enum phy_type_e {
    PHY_TYPE_FEPHY = 0,
    PHY_TYPE_RGMII_MAC = 1,
    PHY_TYPE_RGMII_PHY = 2,
    PHY_TYPE_RMII_MAC = 3,
    PHY_TYPE_RMII_PHY = 4,
};

enum phy_addr_e {
    INT_PHY_ADDR = 1,   /* embedded GPHY PHY ID */
    EXT_PHY_ADDR = 1,   /* external RGMII PHY ID */
};
#else
enum phy_type_e {
    PHY_TYPE_PHY = 0,
...
struct rtl8168_private {
	...
	u8 hw_fail;	/* 0: normal, 1: failed */
	u8 acp_enable;  /* 0: disable, 1: enable */
#if defined(CONFIG_TARGET_RTD1319)
	uint8_t voltage; /* 1:1.8V, 2: 2.5V, 3:3.3V */
...
static void rtl8168_get_mac_address(struct rtl8168_private *tp)
{
    ...
    if (!eth_env_get_enetaddr("ethaddr", env_enetaddr))
        string_to_enetaddr(default_enetaddr, env_enetaddr);
...
#if defined(CONFIG_TARGET_RTD1319)
    phy_status = mac_ocp_read(tp, 0xde40);
    switch (tp->phy_type) {
    case PHY_TYPE_FEPHY:
        /* do nothing */
        break;
    case PHY_TYPE_RMII_MAC:
    case PHY_TYPE_RMII_PHY:
        /* adjust RMII interface setting, speed */
        tmp = mac_ocp_read(tp, 0xea30) & ~(BIT_6 | BIT_5);
        ...
        if ((phy_status & BIT_3) == 0)  /* ETN spec, half duplex */
            tmp &= ~BIT_4;
        else    /* ETN spec, full duplex */
            tmp |= BIT_4;
        mac_ocp_write(tp, 0xea30, tmp);
        break;
    case PHY_TYPE_RGMII_MAC:
    case PHY_TYPE_RGMII_PHY:
        /* adjust RGMII interface setting, duplex */
        tmp = mac_ocp_read(tp, 0xea34) & ~(BIT_4 | BIT_3);
        switch (phy_status & 0x0030) { /* link speed */
        case 0x0000:    /* 10M, ETN spec, RGMII clock speed = 2.5MHz */
            break;
        case 0x0010:    /* 100M, ETN spec, RGMII clock speed = 25MHz */
...
#if defined(CONFIG_TARGET_RTD1319)
    while (ptr != NULL) {
        if (strcmp(ptr, "rgmii1") == 0)
            tp->phy_type = PHY_TYPE_RGMII_MAC;
        else if (strcmp(ptr, "rgmii2") == 0)
            tp->phy_type = PHY_TYPE_RGMII_PHY;
        else if (strcmp(ptr, "rmii1") == 0)
            tp->phy_type = PHY_TYPE_RMII_MAC;
        else if (strcmp(ptr, "rmii2") == 0)
            tp->phy_type = PHY_TYPE_RMII_PHY;
        else if (strcmp(ptr, "fephy") == 0)
            tp->phy_type = PHY_TYPE_FEPHY;
            ...
            tp->voltage = 3;
        else if (strcmp(ptr, "txd0") == 0)
            tp->tx_delay = 0;
        else if (strcmp(ptr, "txd1") == 0)
            tp->tx_delay = 1;
        else if (strcmp(ptr, "rxd0") == 0)
            tp->rx_delay = 0;
        else if (strcmp(ptr, "rxd1") == 0)
            tp->rx_delay = 1;
        else if (strcmp(ptr, "rxd2") == 0)
            tp->rx_delay = 2;
        else if (strcmp(ptr, "rxd3") == 0)
            tp->rx_delay = 3;
        else if (strcmp(ptr, "rxd4") == 0)
            tp->rx_delay = 4;
        else if (strcmp(ptr, "rxd5") == 0)
            tp->rx_delay = 5;
        else if (strcmp(ptr, "rxd6") == 0)
            tp->rx_delay = 6;
...
static void r8168_reset_phy_gmac(void)
{
    ...
    tmp = rtd_inl(ISO_ETN_DBUS_CTRL);
    tmp &= ~BIT_0;
    rtd_outl(ISO_ETN_DBUS_CTRL, tmp);
#endif /* RTK_FPGA */
...
static void r8168_mdio_init(struct rtl8168_private *tp)
{
    ...
    printf("wait %d ms for GMAC interrupt ready. MAC0 = 0x%x\n",
        tmp, RTL_R8(MAC0));
    ...
    case PHY_TYPE_RGMII_PHY:
        ...
        if (tmp < 2000)
            printf("found.\n");
        printf("wait %d ms for RGMII PHY ready. PHY REG 31 = 0x%x\n",
            tmp, mdio_read(tp, 31));

        /* set RGMII RX delay = 0 */
        tmp = mac_ocp_read(tp, 0xea36);
        tmp &= ~(BIT_0);    /* rg_clk_en */
        mac_ocp_write(tp, 0xea36, tmp);

        tmp = mac_ocp_read(tp, 0xea34);
        tmp &= ~(BIT_14 | BIT_9| BIT_8 | BIT_6);
        mac_ocp_write(tp, 0xea34, tmp);

        /* external PHY RGMII timing tuning, rg_rgmii_id_mode = 1 (default) */
        mdio_write(tp, 31, 0x0d08);
        mdio_write(tp, 21, (mdio_read(tp, 21) | BIT_3));
        ...
        mdio_write(tp, 31, 0x0d08);
        mdio_write(tp, 17, (mdio_read(tp, 17) | BIT_8));

        /* ISO spec, data path is set to RGMII */
        tmp = mac_ocp_read(tp, 0xea30);
        tmp &= ~(BIT_0);
        mac_ocp_write(tp, 0xea30, tmp);
        ...
        break;
    default:
        printf("unsupport output mode (%d) in FPGA\n", tp->phy_type);
    }
}
#else
/* for real chip */
static void r8168_load_otp_content(struct rtl8168_private *tp)
{
    ...
    /* enable 0x980174FC[4] */
    /* R-K 0x980174FC[3:0] */
    otp = rtd_inl(EFUSE_OTP_REG) & (0x1f << 0);
    if (otp & BIT_4) {
        tmp = otp ^ R_K_DEFAULT;
        mdio_write(tp, 31, 0xbc0);
        mdio_write(tp, 20,
            ((mdio_read(tp, 20) & ~(0x1f << 0)) | tmp));
    }

    /* Amp-K 0x98017500[7:0] */
    otp = rtd_inl(EFUSE_OTP_REG + 0x4) & (0xff << 0);
    tmp = otp ^ IDAC_FINE_DEFAULT;
    mdio_write(tp, 31, 0xbc0);
    mdio_write(tp, 23,
        ((mdio_read(tp, 23) & ~(0xff << 0)) | tmp));
}
...
static void r8168_phy_sram_table(struct rtl8168_private *tp)
{
}

static void r8168_patch_phy_uc_code(struct rtl8168_private *tp)
{
}
...
static void r8168_mdio_init(struct rtl8168_private *tp)
{
    ...
    /* printf("wait %d ms for GPHY interrupt. UMSK_ISR = 0x%x\n",
...
#if defined(CONFIG_TARGET_RTD1319)
    /* fix A00 known issue */
    if (get_rtd1xxx_cpu_revision() == RTD1xxx_CHIP_REVISION_A00) {
        /* fix AFE RX power as 1 */
        mdio_write(tp, 31, 0x0bd0);
        mdio_write(tp, 21, 0x7201);
    }
#endif
    ...
    switch (tp->phy_type) {
    case PHY_TYPE_FEPHY:
        /* move to r8168_pinmux_init() and r8168_initialize() */
        break;
    case PHY_TYPE_RGMII_MAC:
        printf("%s:%d: FIXME! PHY_TYPE_RGMII_MAC\n", __func__, __LINE__);
        break;
    case PHY_TYPE_RMII_MAC:
    case PHY_TYPE_RMII_PHY:
        printf("%s:%d: FIXME! PHY_TYPE_RMII\n", __func__, __LINE__);
        break;
    case PHY_TYPE_RGMII_PHY:
        #if 0 /* move to r8168_pinmux_init() */
        /* FPGA is fixed to 1.8V, DP=100, DN=100 */
...
static void r8168_pinmux_init(struct rtl8168_private *tp)
{
    ...
    case PHY_TYPE_FEPHY:
        ...
        /* output current: 4mA */
        tmp = rtd_inl(ISO_TESTMUX_PFUNC9);
        tmp &= ~(0xff << 16);
        rtd_outl(ISO_TESTMUX_PFUNC9, tmp);
...
static void r8168_reset_phy_gmac(void)
{
    ...
    tmp = rtd_inl(0x9804f188);
    tmp &= ~BIT_0;
    rtd_outl(0x9804f188, tmp);
    ...
    /* set int PHY addr = 1 */
    /* set ext PHY addr = 3 */
    rtd_outl(0x9804f180, 0x04616713);
#elif defined(CONFIG_TARGET_RTD1619B)
    /* reg_0x9800705c = 0x00216703 */
    /* ISO spec, default value and specify internal/external PHY ID as 1 */
    tmp = rtd_inl(ISO_POWERCUT_ETN);
...
static void r8168_pll_clock_init(struct rtl8168_private *tp)
{
    ...
    tmp = rtd_inl(0x9804e128);
    tmp |= BIT_9;
    rtd_outl(0x9804e128, tmp);

    /* 0x9814e018[16]=0x1, 0x9804e018[20]=0x1 */
    /* Switch to ETN LED location 1 */
    tmp = rtd_inl(0x9814e018);
    tmp |= BIT_16 | BIT_20;
    rtd_outl(0x9814e018, tmp);

    tmp = rtd_inl(0x9804f194);
    printf("0x9804f194 = 0x%08x\n", tmp);
    mdelay(10);
#endif /* CONFIG_TARGET_RTD1625 */
    ...
    tmp = 0;
    while (0x1 != (rtd_inl(ISO_PLL_WDOUT) & BIT_0)) {
        tmp += 1;
        mdelay(1);
#if defined(CONFIG_TARGET_RTD1625)
        if (tmp >= 100) {
...
static void r8168_load_otp_content(struct rtl8168_private *tp)
{
	...
	otp = (rtd_inl(EFUSE_OTP_REG + 0x4) & (0x7 << 0)) >> 0;
	tmp = otp ^ RS_DEFAULT;
	mdio_write(tp, 31, 0xbd0);
	mdio_write(tp, 21, ((mdio_read(tp, 21) & ~(0x7 << 13)) | (tmp << 13)));

	/* Set RSW (for RX) from OTP 0x98032504[5:3] */
	otp = (rtd_inl(EFUSE_OTP_REG + 0x4) & (0x7 << 3)) >> 3;
	tmp = otp ^ RSW_DEFAULT;
	mdio_write(tp, 31, 0xbd0);
	mdio_write(tp, 20, ((mdio_read(tp, 20) & ~(0x7 << 13)) | (tmp << 13)));

	/* Set idac_fine_mdi from OTP 0x98032500[4:0] */
	otp = (rtd_inl(EFUSE_OTP_REG) & (0x1F << 0)) >> 0;
	tmp = otp ^ IDAC_FINE_MDI_DEFAULT;
	mdio_write(tp, 31, 0xbc0);
	mdio_write(tp, 23, ((mdio_read(tp, 23) & ~(0x1F << 0)) | (tmp << 0)));

	/* Set idac_fine_mdix from OTP 0x98032500[9:5] */
	otp = (rtd_inl(EFUSE_OTP_REG) & (0x1F << 5)) >> 5;
	tmp = otp ^ IDAC_FINE_MDIX_DEFAULT;
	mdio_write(tp, 31, 0xbc0);
	mdio_write(tp, 23, ((mdio_read(tp, 23) & ~(0x1F << 8)) | (tmp << 8)));
}
...
static void r8168_phy_iol_tuning(struct rtl8168_private *tp)
{
	...
	tmp = 0x30 << 4;
	mdio_write(tp, 31, 0xa81);
	mdio_write(tp, 23, (mdio_read(tp, 23) & ~(0x3F << 4)) | tmp);
}
#elif defined(CONFIG_TARGET_RTD1619B)
static void r8168_load_otp_content(struct rtl8168_private *tp)
{
	...
	/* R-K 0x9803_2504[3:0] */
	otp = rtd_inl(EFUSE_OTP_REG) & 0xF;
	tmp = otp ^  R_K_DEFAULT;
	/* Set tapbin_p3 & tapbin_p2 */
	mdio_write(tp, 31, 0x0bcf);
	mdio_write(tp, 18, ((tmp << 8) | tmp));
	/* Set tapbin_p1 & tapbin_p0 */
	mdio_write(tp, 19, ((tmp << 8) | tmp));
	/* Set tapbin_pm_p3 & tapbin_pm_p2 */
	mdio_write(tp, 20, ((tmp << 8) | tmp));
	/* Set tapbin_pm_p1 & tapbin_pm_p0 */
	mdio_write(tp, 21, ((tmp << 8) | tmp));

	/* Amp-K 0x9803_2504[31:16] */
	otp = (rtd_inl(EFUSE_OTP_REG) & (0xffff << 16)) >> 16;
	tmp = otp ^ AMP_K_DEFAULT;
	package_code = (rtd_inl(OTP_BOND_ID) & (0x3 << 12)) >> 12;
	ver = (rtd_inl(EFUSE_OTP_REG2) & (0x3 << 16)) >> 16;
	/* hotfix for rtd1315c (package_code == 0x2) w/ amp-k pattern version 0 */
	if ((package_code == 0x2) && (ver == 0x0))
		tmp = (tmp & ~(0xFF << 0)) | (0x22 << 0);
	mdio_write(tp, 31, 0x0bca);
	mdio_write(tp, 22, tmp);

	/* RC-K 0x9803_2508[15:0] */
	otp = rtd_inl(EFUSE_OTP_REG + 0x4) & 0xffff;
	tmp = otp ^ RC_K_DEFAULT;
	mdio_write(tp, 31, 0x0bcd);
	mdio_write(tp, 22, tmp); /* len_p[3:0] */
	mdio_write(tp, 23, tmp); /* rlen_p[3:0] */
}
...
static void r8168_phy_iol_tuning(struct rtl8168_private *tp)
{
    ...
    tmp = 0xb4 << 4;
    mdio_write(tp, 31, 0xbc0);
    mdio_write(tp, 17, (mdio_read(tp, 17) & ~(0xff << 4)) | tmp);

       /* Set LD_COMP to 0x0 */
    tmp = 0 << 9;
    mdio_write(tp, 31, 0xbc0);
    mdio_write(tp, 23, (mdio_read(tp, 23) & ~(0x3 << 9)) | tmp);

    /* set lock main */
    tmp = 0x1 << 1;
    mdio_write(tp, 31, 0x0a46);
    mdio_write(tp, 21, mdio_read(tp, 21) | tmp);
    /* wait until locked */
    tmp = 0;
    mdio_write(tp, 31, 0x0a60);
    while (0x1 != (mdio_read(tp, 16) & (0xff << 0))) {
        tmp += 10;
        udelay(10);
        if (tmp >= 1000) {
            printf("Ethernet PHY is not locked (0x1), current = 0x%02x\n",
                   (mdio_read(tp, 16) & (0xff << 0)));
            break;
        }
    }
    //printf("Ethernet PHY: wait %d ns to start IOL tuning\n", tmp);

    /* Change green table default LDVBIAS to 0x66 */
    tmp = 0x66 << 8;
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x8049);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table 10M LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x8050);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table 100M short LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x8057);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table 100M long LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x805e);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table GIGA short LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x8065);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table GIGA middle LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x806c);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);

    /* Change green table GIGA long LDVBIAS to 0x66 */
    mdio_write(tp, 31, 0x0a43);
    mdio_write(tp, 27, 0x8073);
    mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0xff << 8)) | tmp);
    ...
    package_code = (rtd_inl(OTP_BOND_ID) & (0x3 << 12)) >> 12;
    ver = (rtd_inl(EFUSE_OTP_REG2) & (0x3 << 16)) >> 16;
    /* rtd1315c (package code == 0x2) or other chip w/ amp-k pattern version 1 */
    if ((package_code == 0x2) || (ver == 0x1)){
        /* Change LD_CMFB to 0x2 */
        tmp = 0x2 << 12;
        mdio_write(tp, 31, 0xbc0);
        mdio_write(tp, 23, (mdio_read(tp, 23) & ~(0x3 << 12)) | tmp);

        /* Change green table default LDVDC to 0x5 */
        tmp = 0x5 << 8;
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x804D);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table 10M LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x8054);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table 100M short LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x805B);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table 100M long LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x8062);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table Giga short LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x8069);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table Giga middle LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x8070);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);

        /* Change green table Giga long LDVDC to 0x5 */
        mdio_write(tp, 31, 0x0a43);
        mdio_write(tp, 27, 0x8077);
        mdio_write(tp, 28, (mdio_read(tp, 28) & ~(0x7 << 8)) | tmp);
    }
    ...
    tmp = 0x0 << 1;
    mdio_write(tp, 31, 0x0a46);
    mdio_write(tp, 21, (mdio_read(tp, 21) & ~(0x1 << 1)) | tmp);
}
#endif
...
static void r8168_pinmux_init(struct rtl8168_private *tp)
{
    ...
    /* ISO spec, set data path to GPHY */
    tmp = mac_ocp_read(tp, 0xea30);
    tmp &= ~(BIT_0);
    mac_ocp_write(tp, 0xea30, tmp);
    ...
    /* select ETN LED */
    tmp = rtd_inl(ISO_TESTMUX_MUXPAD2);
    tmp &= ~((0x3 << 29) | (0x7 << 11) | (0x7 << 8));
    tmp |= (0x1 << 29) | (0x1 << 11) | (0x1 << 8);
    rtd_outl(ISO_TESTMUX_MUXPAD2, tmp);

    /* output current: 4mA */
    tmp = rtd_inl(ISO_TESTMUX_PFUNC12);
    tmp &= ~(0x7fff << 0);
