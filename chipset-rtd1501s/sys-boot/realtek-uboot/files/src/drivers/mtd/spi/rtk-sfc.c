...
#if defined(CONFIG_TARGET_RTD1625)
	regbase = ioremap(0x9804F230, 0x8);
#else
...
#if defined(CONFIG_TARGET_RTD1625)
        /* CLK */
	rtk_set_reg(regbase + 0x4, 5, 1, vdd);
	rtk_set_reg(regbase + 0x4, 4, 1, vdd);
	rtk_set_reg(regbase + 0x4, 2, 1, 0);
	rtk_set_reg(regbase + 0x4, 1, 1, 1);
	rtk_set_reg(regbase + 0x4, 0, 1, 1);

        /* CSN */
	rtk_set_reg(regbase, 31, 1, vdd);
	rtk_set_reg(regbase, 30, 1, vdd);
	rtk_set_reg(regbase, 28, 1, 0);
	rtk_set_reg(regbase, 27, 1, 1);
	rtk_set_reg(regbase, 26, 1, 1);

        /* MOSI */
	rtk_set_reg(regbase + 0x4, 17, 1, vdd);
	rtk_set_reg(regbase + 0x4, 16, 1, vdd);
	rtk_set_reg(regbase + 0x4, 14, 1, 0);
	rtk_set_reg(regbase + 0x4, 13, 1, 1);
	rtk_set_reg(regbase + 0x4, 12, 1, 1);

        /* MISO */
	rtk_set_reg(regbase + 0x4, 11, 1, vdd);
	rtk_set_reg(regbase + 0x4, 10, 1, vdd);
	rtk_set_reg(regbase + 0x4, 8, 1, 0);
	rtk_set_reg(regbase + 0x4, 7, 1, 1);
	rtk_set_reg(regbase + 0x4, 6, 1, 1);
#else
	/* CLK */
...
#if defined(CONFIG_TARGET_RTD1625)
	regmap_write(host->sb2, SFC_SCK, 0x00000003);
#else
...
#endif /* __UBOOT__ */
{
	...
	config.r_start = 0x9814b000;
	config.r_size = 0x50;
	config.reg_offset_shift = 0;
	config.width = REGMAP_SIZE_32;
	host->sb2 = devm_regmap_init(dev, NULL, NULL, &config);
#else
