...
#define rstn_usb_phy4 		(0x1 << 30) // u3host u2phy
#define rstn_usb_phy3 		(0x1 << 29) // u2host u2phy
#define rtk_usb_u3phy2_mdio	(0x1 << 28) // u3host u3phy
#define rtk_usb_u3phy2 		(0x1 << 27) // u3host u3phy
...
#define clk_en_usb_p3 		(0x1 << 1) // p3
#define clk_en_usb_p4 		(0x1 << 0) // p4
...
static void _rtk_usb_clock_init(void)
{
	...
	reset_pll_flag |= rstn_usb_phy3 | rstn_usb_phy4 |
		    rtk_usb_u3phy2 | rtk_usb_u3phy2_mdio;
	clk_en_mac_flag |= clk_en_usb_p3 | clk_en_usb_p4;
...
#if defined(CONFIG_TARGET_RTD1625)
#define DISABLE_BC 0x3800
#define AUTO_INVERT_SETTING (0x5 << 27)
#define AUTO_INVERT_EN BIT(26)

	printf("Kent fix bc mododule\n");
	writel(DISABLE_BC, 0x98013330);
	writel(DISABLE_BC, 0x98013d30);
	writel(DISABLE_BC, 0x98013f30);

	printf("Kent disable port1 and port3 suspend\n");
	writel(~BIT(8) & readl(0x98013a00), 0x98013a00);
	writel(~BIT(8) & readl(0x98013c00), 0x98013c00);
	writel(~BIT(6) & readl(0x9815c204), 0x9815c204);
	writel(~BIT(6) & readl(0x9815c208), 0x9815c208);

	printf("Kent auto invert clk for port1\n");
	writel((AUTO_INVERT_EN | AUTO_INVERT_SETTING), 0x98013934);
	debug("0x98013934=0x%x\n", readl(0x98013934));

	printf("Kent disable port 4 u3 phy\n");
	writel(BIT(13) | readl(0x98013800), 0x98013800);
	writel(0x1, 0x9801380c);
	writel(BIT(8) | readl(0x98013860), 0x98013860);

	writel(~BIT(1) & readl(0x98013928), 0x98013928);

	debug("0x9815c11c=0x%x\n", readl(0x9815c11c));
	//writel(BIT(26) | readl(0x9815c11c), readl(0x9815c11c));
	writel(0x0404198A, 0x9815c11c);
	debug("0x9815c11c=0x%x\n", readl(0x9815c11c));
#else
