...
extern void rtk_hexdump( const char * str, void * pBuf, unsigned int length );
...
int AES_CBC_decrypt(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int key[4], unsigned int iv[4], unsigned int mode)
{
	...
	if (iv != NULL) {
		dscpt->ini_key[0] = SWAPEND32(iv[0]);
		dscpt->ini_key[1] = SWAPEND32(iv[1]);
		dscpt->ini_key[2] = SWAPEND32(iv[2]);
		dscpt->ini_key[3] = SWAPEND32(iv[3]);
	}
...
int MCP_AES_CBC_decrypt(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int key[8], unsigned int iv[4], unsigned int key_len)
{
	unsigned int mode = 0;


	if (key_len == AES256_KEY_LENGTH) {
		mode = 0x55;
	} else if (key_len == AES192_KEY_LENGTH) {
		mode = 0x5d;
	} else if (key_len == AES128_KEY_LENGTH) {
		mode = 0x45;
	}
	if (mode) mode |= 0x3000; // key from DDR

	return AES_CBC_decrypt(src_addr, length, dst_addr, key, iv, mode);
}
...
int do_RSA(unsigned char* signature_addr, unsigned char* rsa_addr, unsigned char *output_addr)
{
	...
	while (tcnt >= 0) {
		...
		if ((value & RSA_STAT_DONE) == RSA_STAT_DONE) {
			break; // RSA done without error
		} else if (value & RSA_STAT_ERROR) {
			printf("RSA status error, and reset RSA status register.\n");
			REG32(RSA_CTRL4) = 0;
			mdelay(1);
			return -1;
		}
...
int Verify_SHA256_hash(unsigned char * src_addr, unsigned int length, unsigned char * ref_sha256, unsigned int do_recovery, unsigned char *rsa_key_addr)
{
	...
	ret = MCP_SHA256_hash_hwpadding(src_addr, length, hash1, NULL, 1);
...
void cmd_bl31_hwrsa(void * signature_addr, void * rsa_key_addr, void * output_addr)
{
	//process hw rsa
	struct arm_smccc_res res = {0};

	arm_smccc_smc(0x8400ff07, (unsigned long)signature_addr, (unsigned long)rsa_key_addr, (unsigned long)output_addr, 0, 0, 0, 0, &res);
}

int Verify_SHA256_hash_SMC(unsigned char *src_addr, unsigned int length, unsigned char *ref_sha256, unsigned char *rsa_key_addr, unsigned int hw_padding)
{
	int ret = 0;
	unsigned char hash1[32]; // hash value calculated by CP engine
	unsigned char hash2[32]; // hash value recovery from RSA signature
	...
	flush_cache((u32)(uintptr_t)src_addr, length);
	ret = MCP_SHA256_hash_hwpadding(src_addr, length, hash1, NULL, hw_padding);
	...
	rsa_key_info = malloc(sizeof(rsa_key_t));
	rsa_key_info->type = 0; // normal key
	rsa_key_info->key = sys_rsa_key_address;
	flush_cache((unsigned int)(uintptr_t)rsa_key_info, sizeof(rsa_key_t));
	cmd_bl31_hwrsa(signature_key_address, (void *)rsa_key_info, hash2);
...
int MCP_SHA256_hash_hwpadding(unsigned char * src_addr, unsigned int length, unsigned char *dst_addr, unsigned int iv[8], unsigned int hw_padding)
{
	int ret = 0;
#ifdef MCP_DEBUG
	rtk_hexdump("src_addr", src_addr, length);
	rtk_hexdump("dst_addr", dst_addr, sizeof(unsigned int)*8);
#endif

	if (!hw_padding)
		rtd_outl(MCP_CTRL1, rtd_inl(MCP_CTRL1) | 0x800); //turn off hw_padding
	else
		rtd_outl(MCP_CTRL1, rtd_inl(MCP_CTRL1) & (~0x800));

	flush_cache((u32)(uintptr_t)src_addr, length);
	ret = MCP_SHA256_hash(src_addr, length, dst_addr, iv);

	if (ret) {
		printf("[ERR] %s: caculate hash256 fail(%d)\n", __FUNCTION__, ret);
		return -1;
	}

	flush_cache((u32)(uintptr_t)dst_addr, 32);
#ifdef MCP_DEBUG
	rtk_hexdump("dst_addr", dst_addr, sizeof(unsigned int)*8);
#endif
