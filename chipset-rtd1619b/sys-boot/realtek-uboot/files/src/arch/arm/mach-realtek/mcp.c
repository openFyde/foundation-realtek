...
int AES_CBC_decrypt(unsigned char * src_addr, unsigned int length, unsigned char * dst_addr, unsigned int key[4], unsigned int iv[4])
{
	...
	if (iv != NULL) {
		dscpt->ini_key[0] = iv[0];
		dscpt->ini_key[1] = iv[1];
		dscpt->ini_key[2] = iv[2];
		dscpt->ini_key[3] = iv[3];
	}
...
{
	//process hw rsa
	asm volatile("mov x3, %0" : :"r"(output_addr): "cc");
	asm volatile("mov x2, %0" : :"r"(rsa_key_addr): "cc");
	asm volatile("mov x1, %0" : :"r"(signature_addr): "cc");
	asm volatile("ldr x0, =0x8400ff07" : : : "cc"); //function id
	asm volatile("isb" : : : "cc");
